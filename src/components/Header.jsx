...
const Header = ({ onSearch }) => {
    ...
    const fetchNavItems = async () => {
      try {
        ...
        if (catRes.status === "fulfilled" && catRes.value?.data?.length > 0) {
          setCategoriesList(catRes.value.data.map((cat) => ({ name: cat.name, path: `/collections/${cat.slug}` })));
        }
        if (colRes.status === "fulfilled" && colRes.value?.data?.length > 0) {
          setCollectionsList(colRes.value.data.map((col) => ({ name: col.name, path: `/collections/${col.handle}` })));
        }
      } catch (_) {}
    };
    fetchNavItems();
  ...
  return (
    <header className="main-header">
      ...
      <div className="hdr-top-bar">
        <div className="container-fluid">
          <div className="hdr-top-inner">
            ...
            <form className="hdr-search-form" onSubmit={handleSearchSubmit}>
              <input
                type="text"
            ...
            <div className="hdr-actions">
              {/* Account */}
              {user ? (
                <Link href="/profile" className="hdr-icon-btn" aria-label="My Profile" title={`Signed in as ${user.name || user.email}`}>
                ...
                <Link href="/pages/login" className="hdr-icon-btn hdr-account-icon" aria-label="Account">
                  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />
                    <circle cx="12" cy="7" r="4" />
                  </svg>
              ...
              {/* Wishlist */}
              <Link href="/pages/wishlist" className="hdr-icon-btn hdr-wishlist-icon relative" aria-label="Wishlist">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              ...
              {/* Cart */}
              <button className="hdr-icon-btn relative" onClick={() => setCartOpen(true)} aria-label="Cart">
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* ── SECONDARY NAV BAR: Shop | Categories | Collections (desktop only) ── */}
      <div className="hdr-nav-bar hidden md:block">
        <div className="container-fluid">
          <nav className="hdr-nav-inner">
      ...
      {mobileMenuOpen && (
        <div
          className="fixed inset-0 bg-black/50 z-[998] backdrop-blur-sm md:hidden"
      ...
      {mobileMenuOpen && (
        ...
        >
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-neutral-100 flex-shrink-0">
            <Logo height={40} />
            <button onClick={() => setMobileMenuOpen(false)} aria-label="Close menu" className="p-2 text-neutral-600 hover:text-black">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <nav className="flex-grow overflow-y-auto px-6 py-2">
            <ul className="flex flex-col">
              ...
              <li style={{ borderBottom: '1px solid #f3f4f6' }}>
                <button onClick={() => setShopExpanded(!shopExpanded)} className="w-full flex items-center justify-between text-base font-semibold text-neutral-800 hover:text-black transition-colors py-4">
                  <span>Shop</span>
              ...
              <li style={{ borderBottom: '1px solid #f3f4f6' }}>
                <button onClick={() => setCategoriesExpanded(!categoriesExpanded)} className="w-full flex items-center justify-between text-base font-semibold text-neutral-800 hover:text-black transition-colors py-4">
              ...
              <li style={{ borderBottom: '1px solid #f3f4f6' }}>
                <button onClick={() => setCollectionsExpanded(!collectionsExpanded)} className="w-full flex items-center justify-between text-base font-semibold text-neutral-800 hover:text-black transition-colors py-4">
              ...
              <li style={{ borderBottom: '1px solid #f3f4f6' }}>
                <Link href="/pages/wishlist" onClick={() => setMobileMenuOpen(false)} className="w-full flex items-center justify-between text-base font-semibold text-neutral-800 hover:text-black transition-colors py-4">
                  Wishlist
                </Link>
              </li>

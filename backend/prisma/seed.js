...
async function main() {
  ...
  if (!fs.existsSync(jsonPath)) {
    console.warn(
      `⚠️  products.json not found at ${jsonPath}. Skipping product seed.`,
    );
    console.warn(
      "   Place products.json at: src/data/products.json relative to workspace root",
    );
    return;
  }
  ...
  for (const p of products) {
    ...
    const productCollections = ["all"];
    const tags = p.tags || [];
    if (
      tags.some((t) =>
        ["bestseller", "best-seller", "trending"].includes(t.toLowerCase()),
      )
    ) {
    ...
    if (
      tags.some((t) =>
        ["new", "new-arrival", "whats-new"].includes(t.toLowerCase()),
      )
    ) {
  ...
  console.log(
    `✅ Products seeded: ${seeded}, skipped (already exist): ${skipped}`,
  );

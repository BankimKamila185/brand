...
router.get('/', asyncHandler(async (_req, res) => {
  const collections = await db.collection.findMany({
    where: { isActive: true },
    select: {
      id: true, name: true, handle: true, description: true, imageUrl: true,
...
router.get('/:handle', asyncHandler(async (req, res) => {
  ...
  const [total, products] = await Promise.all([
    db.product.count({ where: { isActive: true, collections: { some: { collectionId: collection.id } } } }),
    db.product.findMany({
      where: { isActive: true, collections: { some: { collectionId: collection.id } } },
      select: {
        id: true, title: true, handle: true, productType: true, tags: true,
        variants: {
          where: { isActive: true },
          select: { id: true, title: true, option1: true, price: true, comparePrice: true,
            inventory: { select: { quantity: true } } },
          orderBy: { position: 'asc' }, take: 10,
        },
        images: { select: { src: true, altText: true }, orderBy: { position: 'asc' }, take: 2 },
      },
      skip, take: limit,

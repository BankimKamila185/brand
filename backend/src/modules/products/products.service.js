    ...
    const product = await db.$transaction(async (tx) => {
      const created = await tx.product.create({
        data: {
          ...
          fabric: data.fabric || "100% Super Combed Cotton",
          gsm: data.gsm || "240 GSM Heavyweight",
          fit: data.fit || "Oversized Drop-Shoulder",
          printType: data.printType || "High-Density HD Print",
    ...
    await db.$transaction(async (tx) => {
      ...
      const updatedProduct = await tx.product.update({
        ...
        data: {
          ...
          gsm: data.gsm !== undefined ? data.gsm : undefined,
          fit: data.fit !== undefined ? data.fit : undefined,
          printType: data.printType !== undefined ? data.printType : undefined,

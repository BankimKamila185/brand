...
const productListSelect = {
  ...
  variants: {
    ...
    select: {
      ...
      price: true, comparePrice: true, position: true,
      inventory: { select: { quantity: true, reserved: true } },
    },
    orderBy: { position: 'asc' as const },
  },
  images: {
    select: { id: true, src: true, altText: true, width: true, height: true, position: true },
    orderBy: { position: 'asc' as const },
    take: 2,
  },
...
const productDetailSelect = {
  ...
  variants: {
    ...
    select: {
      ...
      weight: true, position: true, taxable: true, requiresShipping: true,
      inventory: { select: { quantity: true, reserved: true } },
    },
    orderBy: { position: 'asc' as const },
  },
  images: {
    select: {
      id: true, src: true, altText: true,
      width: true, height: true, position: true, variantId: true,
    },
    orderBy: { position: 'asc' as const },
  },
} satisfies Prisma.ProductSelect;
...
export const productsService = {
  async list(query: ProductQueryInput) {
    const { page, limit, category, collection, productType, tags,
      minPrice, maxPrice, sortBy, sortOrder, search, available } = query;
  ...
  async search(query: string, limit = 10) {
    const products = await db.product.findMany({
      ...
      select: {
        id: true, title: true, handle: true, productType: true,
  ...
  async create(data: CreateProductInput) {
    const existing = await db.product.findUnique({ where: { handle: data.handle } });
    if (existing) throw new AppError('A product with this handle already exists', 409);
